...
module.exports.run = async (client, message, args, settings) => {
    ...
    if (user) {
        ...
        await user.send(`Vous avez été banni définitivement du serveur **${message.guild.name}** par <@${message.author.id}> pour la raison:\n\`${reason}\``)
        .catch(console.error);
        ...
        message.channel.send({ 
            embeds: [
                new MessageEmbed()
                .setDescription(`**<@${user.id}>** a été banni définitivement par <@${message.author.id}> pour la raison:\n\`${reason}\``)
            ]
        });
        message.delete();
    } else {
